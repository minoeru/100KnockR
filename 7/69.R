...
ui <- fluidPage(
   ...
   sidebarLayout(
      sidebarPanel(
        textInput("string1", "Artist Name", ""),
        textInput("string2", "Artist Aliases Name", ""),
        textInput("string3", "Tags Value", ""),
...
server <- function(input, output) {
  observeEvent(input$result,{
    in_name <<- input$string1
    in_aliases_name <<- input$string2
    in_tags_value <<- input$string3
    database2 <<- database
    if(in_name != ""){
      database2 <<- database2[database2$name==in_name,]
    }
    if(in_aliases_name != ""){
      database2 <<- database2[database2$aliases.name==in_aliases_name,]
    }
    if(in_tags_value != ""){
      database2 <<- database2[database2$tags.value==in_tags_value,]
    }
    output$table <- DT::renderDataTable(database2)
  })
}
