  ...
  spoil <- lapply(1:length(tmp$morphs),function(y){
    ...
    if(is.list(tmp2)){ # 複数
      ...
      particle <<- lapply(1:length(tmp2),function(z){
        ...
        # チェック
        if(length(grep("サ変接続",out_text2)) + length(grep("を",out_text2)) == 2) flag2 <<- 1
        particle <- lapply(1:length(out_text2),function(n){
          if(length(grep("サ変接続",out_text2[n])) != 0) {
      ...
      ifelse(flag2 == 1,subject <<- paste0(subject,"を"),return())
      flag <- flag[!is.na(flag)]
      flag <- flag[-which(particle %in% "を")]
      particle <- particle[-which(particle %in% "を")]

      # 係り元の助詞がなければ終了
      if(is.null(particle)) return()
      # 係るのがないと終了
      if(length(flag) == 0) return()
      # 50音順にソート
    ...
    paste(paste0(subject,verb),paste(particle,collapse = " "),paste(ans,collapse = " ")) %>% print() %>% return()
  }) %>% unlist() %>% return()
}) %>% unlist()
