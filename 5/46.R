  ...
  spoil <- lapply(1:length(tmp$morphs),function(y){
    ...
    if(is.list(tmp2)){ # 複数
      ...
      spout2 <<- lapply(1:length(tmp2),function(z){
        ...
        spout2 <- lapply(1:length(out_text2),function(n){
          if(length(grep("助詞",out_text2[n])) != 0){
            flag[z] <<- tmp2[z]
      ...
      # 係り元の助詞がなければ終了
      ifelse(is.null(spout2),return(),ans <<- tmp$morphs[tmp2])
    }
    paste(spout,paste(spout2,collapse = " "),paste(ans,collapse = " ")) %>% print() %>%   return()
  }) %>% unlist() %>% return()
}) %>% unlist()
