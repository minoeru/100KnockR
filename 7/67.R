...
FindAliasesName <- function(x){
  return(database[database$aliases.name==x,])
}

Func67 <- function(path){
  ...
  sample(1:sum,1) %>% as.character(database$aliases.name[.]) %>% FindAliasesName(.)
}
